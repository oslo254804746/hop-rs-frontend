...
test.describe('390px mobile workspace', () => {
  ...
  test('keeps host trust available in More with a mobile-safe reset flow', async ({ page }) => {
    const dock = page.getByRole('navigation', { name: 'Mobile navigation' })
    await dock.getByRole('button', { name: 'More', exact: true }).click()
    const more = page.getByRole('region', { name: 'More navigation' })
    await more.getByRole('link', { name: 'Host trust', exact: true }).click()

    await expect(page).toHaveURL(/\/known-hosts$/)
    await expect(page.getByRole('heading', { name: 'Host trust', level: 1, exact: true })).toBeVisible()
    await expect(dock.getByRole('button', { name: 'More', exact: true })).toHaveAttribute('aria-current', 'page')
    await expect(page.getByRole('list', { name: 'Known Hosts' })).toBeVisible()
    await page.getByRole('complementary', { name: 'Known Host details' })
      .getByRole('button', { name: 'Reset trust', exact: true })
      .click()
    const confirmation = page.getByRole('dialog')
    await expect(confirmation).toContainText('SHA256:')
    await expect(confirmation.locator('.confirm-dialog__actions')
      .getByRole('button', { name: 'Keep trust', exact: true })).toBeFocused()
    await expect(confirmation.getByRole('button', { name: 'Reset trust', exact: true })).toBeVisible()
    await expectNoPageOverflow(page)
  })
})
